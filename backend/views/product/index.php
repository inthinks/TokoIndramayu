...
<?php 
     $toolbars = [
        ['content' =>
            Html::a('<i class="glyphicon glyphicon-plus"></i>', ['product/create'], ['type' => 'button', 'title' => 'Add ' . $this->title, 'class' => 'btn btn-success']) . ' ' .
            Html::a('<i class="fa fa-file-excel-o"></i>', ['product/parsing'], ['type' => 'button', 'title' => 'Parsing Excel ' . $this->title, 'class' => 'btn btn-warning']) . ' ' .
            ...
            Html::a('<i class="glyphicon glyphicon-repeat"></i>', ['product/index','p_reset'=>true], ['data-pjax' => 0, 'class' => 'btn btn-default', 'title' => 'Reset Grid']). ' '
    ...
    $dynagrid = DynaGrid::begin([
                ...
                'options' => ['id' => 'Product'.Yii::$app->user->identity->id] // a unique identifier is important
    ]);
...
if(array==""){
    ...
    if(window.confirm("Are You Sure to delete selected data?")){
        $.ajax({
            type:"POST",
            url:"'.Yii::$app->urlManager->createUrl(['product/delete-all']).'",
            data :{pk:array},
            success:function(){
                location.href="";
            }
