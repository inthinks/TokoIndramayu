...
class UserController extends Controller
{
	...
	/*public function actionCreate()
	{
		...
		if(isset($_POST['User']))
		{
			...
			$dua=$model->password;
			$model->saltPassword=$model->genereateSalt();
			$model->password=$model->hashPassword($dua,$model->saltPassword);
			$model->level_id=3;
			$model->isActive=0;
			$sss;
			if(strlen(trim(CUploadFile::getInstance($model,'avatar'))) > 0)
			{
				$sss=CUploadFile::getInstance($model,'avatar');
